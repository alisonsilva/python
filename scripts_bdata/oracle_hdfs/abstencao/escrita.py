...
from oracle_hdfs.filiadomes.localidade import Localidade
...
class EscritaArquivos(threading.Thread):
    ...
    def __escreve_dados_arquivo(self, conexao, partition, secao, uf):
        raiz_nome_arquivo = Configuration.get_val(section_name='ARQUIVO_ABSTENCAO', val_name='raiz_nome_arquivo')
        caminho = Configuration.get_val(section_name='ARQUIVO_ABSTENCAO', val_name='caminho_destino')
...
def executa_recuperacao_ft_ev_abstencao(uf):
...
if __name__ == '__main__':
    executa_recuperacao_ft_ev_abstencao(3)
